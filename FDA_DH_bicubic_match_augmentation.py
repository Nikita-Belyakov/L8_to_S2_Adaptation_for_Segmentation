...
import torch
class FDA_mask_Transform(torch.nn.Module):
    ...
    def forward(self, x_tif, mask):
        ...
        if self.p>eps:
            ...
            aug_FDA = A.Compose([A.FDA([l8_bicubic.transpose(1,2,0)], p=1, read_fn=lambda x: x)])
            fda_res = aug_FDA(image=l8_DH.transpose(1,2,0))['image']
            fda_res = fda_res.transpose(2,0,1)
            x_tif_masked = inv_mask*x_tif[6:]
            x_tif = x_tif_masked + fda_res*mask_
        return x_tif